...
// Single source of truth for the brand's socials + their glyphs, so the footer
// and the contact card render the exact same icon set and chip treatment.
interface Social {
  label: string;
  href: string;
  icon: ReactNode;
...
// Shared circular "chip" treatment. `tone` adapts the resting colors to the
// surface (dark charcoal footer vs. light cream contact card); the ember hover
// (lift + glow) is identical on both.
export function SocialChips({
  ...
  tone: "dark" | "light";
  /** Optional subset of labels to render, in order. Defaults to all. */
  only?: string[];
  className?: string;
}) {
  const links = only
  ...
  const resting =
    tone === "dark"
      ...
      : "border-ink/10 bg-ink/[0.02] text-warm-muted";
    ...
    <div className={`flex gap-3 ${className}`}>
      {links.map(({ label, href, icon }) => (
        <a
          ...
          className={`flex h-11 w-11 items-center justify-center rounded-full border transition-all duration-300 hover:-translate-y-1 hover:border-ember/60 hover:bg-ember/10 hover:text-ember hover:shadow-[0_8px_24px_-8px] hover:shadow-ember/40 ${resting}`}
        >
          {icon}
        </a>
      ))}
